...
df['Definition']= df['Definition'].replace("Pic thermique:la moyenne interannuelle des températures moyennes des plus chauds 30 jours",
                                           "Pic thermique:la moyenne annuelle des températures moyennes des plus chauds 30 jours")
                                           
df = df.drop(df[df['Definition']=="Index de saisonnalité des températures"].index)
...
df= df.sort_values(['Riv','Station','an'])
df=df.dropna()
df["an"] = df['an'].astype(int)
df['Saison_donnees'] = df['Saison_donnees'].map({'Summer_Data': 'Données_estivales', 'With_WinterData': 'Données_annuelles'})
...
if df_selection.empty:
    ...
    fig = px.bar(df_selection, x='an', y='Valeur_Indice', color='Station', barmode='group',
                 width=600,height=650, template = "seaborn")
fig.update_layout(xaxis=dict(tickvals = df_selection["an"].astype(int)))
